...
use super::typography::{RenderTextParams, TextMeasureParams};
use super::{ConcreteSize, Renderer};
use crate::{
    Border, ColorKind, Font, Layer, LayerOffset, Layout, Line, Rectangle, Result, SolidColor,
};
        ...
        if let Some(debug) = &layout.debug {
            ...
            let outline_layer = Layer {
                rectangle: Some(Rectangle {
                    color: ColorKind::SolidColor(SolidColor::new(0, 0, 0, 0)),
                    border: Some(Border {
                        color: ColorKind::SolidColor(bg.clone()),
                        ..Default::default()
                    }),
                    ..Default::default()
                }),
            ...
            let layout_top_offset = LayerOffset::default();
            ...
            self.render_text(
                ...
                layout_top_label,
                RenderTextParams {
                    color: &ColorKind::SolidColor(fg.clone()),
                    font_size,
            ...
            for (index, layer) in layout.layers.iter().enumerate() {
                ...
                let top_offset = LayerOffset {
                    x: layer.offset.x.max(0),
                    y: layer.offset.y.max(0),
                };
                let top_size = self
