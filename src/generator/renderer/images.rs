...
use crate::{ImgGenError, Layer, PreserveAspect, Result};
...
impl Renderer<'_> {
    ...
    fn load_image(
        ...
        let mut buf = ImageReader::open(path.clone())
            .map_err(|source| ImgGenError::OpenImageFailed {
                path: path.clone(),
                source,
            })?
            .decode()
            .map_err(|source| ImgGenError::DecodeImageFailed {
                path: path.clone(),
                source,
            })?;
    ...
    fn load_svg(
        ...
        let tree = {
            let svg_data = std::fs::read(&path).map_err(|source| ImgGenError::ReadSvgFailed {
                path: path.clone(),
                source,
            })?;
            Tree::from_data(&svg_data, &self.svg_options).map_err(|source| {
                ImgGenError::ParseSvgFailed {
                    path: path.clone(),
                    source,
                }
            })?
        ...
        let mut pixmap = Pixmap::new((og_width * scale_x) as u32, (og_height * scale_y) as u32)
            .ok_or(ImgGenError::SvgScaledToZeroSize { path: path.clone() })?;
        resvg::render(
            &tree,
        ...
        let svg = RgbaImage::from_raw(pixmap.width(), pixmap.height(), Vec::from(pixmap.data()))
            .ok_or(ImgGenError::RasterBufferConversionFailed {
                shape: "svg",
                width: pixmap.width(),
