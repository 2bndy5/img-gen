...
use crate::{
    Border, ColorKind, ImgGenError, Layer, LayerOffset, Result, Size,
...
impl<'a> Renderer<'a> {
    pub fn new(options: Options<'a>, fontsource_client: FontSourceClient) -> Self {
        ...
        font_cx.collection.register_fonts(
            Blob::from(crate::DEFAULT_FONT_ROBOTO_REGULAR_400.to_vec()),
            None,
        );
    ...
    fn render_shape(
        ...
        let mut pixmap = Pixmap::new(shape_size.width, shape_size.height).ok_or(
            ImgGenError::PixmapAllocationFailed {
                shape: "shape",
        ...
        let mut body =
            RgbaImage::from_raw(pixmap.width(), pixmap.height(), Vec::from(pixmap.data())).ok_or(
                ImgGenError::RasterBufferConversionFailed {
                    shape: "shape",
                    width: pixmap.width(),
