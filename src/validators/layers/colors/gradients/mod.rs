...
pub(super) fn parse_color_map_to_gradient<'s>(
    ...
    // first sort the HashMap by values (f64)
    let mut domain = vec![];
    let mut colors = vec![];
    ...
    // if no colors were specified, add black to white as default
    if domain.is_empty() {
        domain.push(0.0);
        colors.push("black");
        domain.push(1.0);
...
impl ColorGradient {
    ...
    pub fn new(spec: Option<Vec<(f32, &str)>>, preset: Option<Presets>) -> Result<Self> {
        ...
        .map_err(|e| ImgGenError::InvalidGradientSpec {
            reason: e.to_string(),
        })?;
